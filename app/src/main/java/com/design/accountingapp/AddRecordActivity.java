...
public class AddRecordActivity extends AppCompatActivity implements View.OnClickListener, CategoryRecyclerAdapter.OnCategoryClickListener {
    ...
    private RecordBean.RecordType type = RecordBean.RecordType.RECORD_TYPE_EXPENSE;//type默认是支出
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        handleDone();//完成
        handleDot();//“.”
        handleTypeChange();//更改支出收入形式

        //消费类型图标选择设定与排版，被选中的图标可以区分于其他图标
        recyclerView = findViewById(R.id.recyclerView);
        adapter = new CategoryRecyclerAdapter(getApplicationContext());
        recyclerView.setAdapter(adapter);
        GridLayoutManager gridLayoutManager = new GridLayoutManager(this, 4);//竖排4个图标
